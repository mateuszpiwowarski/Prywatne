...
// ==== USTAWIENIA ====
const char* WIFI_SSID   = "Security 367/3"; // wpisz swoja nazwę sieci
const char* WIFI_PASS   = "Bae5p4ukeprj"; // wpisz hasło sieci
...
static constexpr uint8_t RELAY1_PIN = 6;  // GPIO6 (OUT) Alarm
static constexpr uint8_t RELAY2_PIN= 7;  // GPIO6 (OUT) Świtło
...
void setup() {
  ...
  digitalWrite(RELAY1_PIN, LOW); // OFF dla active-LOW

    // Przekaźnik aktywny LOW: na start wymuś OFF
  pinMode(RELAY2_PIN, OUTPUT);
  digitalWrite(RELAY2_PIN, LOW); // OFF dla active-LOW
...
// dwa kanały przekaźników w SUPLI (active-LOW => false)
auto r1 = new Supla::Control::Relay(RELAY1_PIN, true);
auto r2 = new Supla::Control::Relay(RELAY2_PIN, true);

// opcjonalnie nazwy kanałów
r1->setInitialCaption("Przekaznik Alarm");
r2->setInitialCaption("Przekaznik Światło");

  // Kanał pomiarowy (GPM)
vmeas = new Supla::Sensor::GeneralPurposeMeasurement();
vmeas->setInitialCaption("Napiecie akumulatora");
