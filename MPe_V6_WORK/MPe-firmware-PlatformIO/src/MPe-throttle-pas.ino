...
void setThrottle()
{
  ...
  if (rideOK)
  {
    ...
    else if ((!thumb_throttle_active || legallimit) && !cruisecontrol && ((cadence > int(EEPROM.readInt(ADR_CAD_MIN_PAS_1 + assistmodeadd)) && checkMinimumHumanPower()) || checkStartupWeightOnPedal())) // PAS
    {
      throttlemode = 3;
    }
    ...
    case 3: // PAS
      ...
      if ((speed >= EEPROM.readInt(ADR_MIN_SPD_PAS_1 + assistmodeadd) && speed >= 0.15) || checkStartupWeightOnPedal())
      {
        ...
        if (legallimit)
        {
          ...
          if (spd_max > int(EEPROM.readInt(ADR_LEGALLIMIT_SPEED)))
          {
            spd_max = EEPROM.readInt(ADR_LEGALLIMIT_SPEED);
            spd_max_min5 = spd_max - 5;
          }

          if (power > int(EEPROM.readInt(ADR_LEGALLIMIT_POWER)))
        ...
        if (legallimit)
        {
          if (set > int(EEPROM.readInt(ADR_LEGALLIMIT_POWER)))
        ...
        if (!legallimit)
        {
          int boost_speed = EEPROM.readInt(ADR_BOOST_SPEED_PAS_1 + assistmodeadd);
          if (zero_cadence_time < EEPROM.readInt(ADR_BOOST_TIME_PAS_1 + assistmodeadd))
          {
            if (int(speed) < boost_speed)
            {
...
{
  int assistmode = EEPROM.readInt(ADR_ASSISTMODE);
  int thr_safe_voltage = EEPROM.readInt(ADR_THR_SAFE_VOLTAGE);

  int16_t pin_thrIn_mV = 0;
  pin_thrIn_mV = ads.readADC_SingleEnded(PIN_THROTTLE_IN) * 0.01875;

  if (
      (getBatVoltage() >= EEPROM.readInt(ADR_LVC) / 10.0) && temperatureOK && !brake && (assistmode > 0) && (getCurrent() >= -((float)EEPROM.readInt(ADR_CUR_PROT))) && (pin_thrIn_mV < thr_safe_voltage)) // 370))
