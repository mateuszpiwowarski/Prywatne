...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  ssd1306_command(SSD1306_SEGREMAP | 0x0); // odwraca kolumny
  ssd1306_command(SSD1306_COMSCANINC);     // odwraca skanowanie wierszy
  ...
  else
  {
    ...
    //Wire.endTransmission();
     //I2c.write(address, registerAddress, *data)
      //I2c.write(_i2caddr, 0x01, control);
      //for(int i = 0; i<127;i++)
      I2c.write((uint8_t)_i2caddr, (uint8_t)0x00, (uint8_t)c);
...
void Adafruit_SSD1306::display(void) {
  //PONIZEJ DODANE NA POTRZEBY KRZACZENIA PO I2C
  ssd1306_command(SSD1306_SETDISPLAYOFFSET);              // 0xD3
  ssd1306_command(0x0);                                   // no offset
  ssd1306_command(SSD1306_SETSTARTLINE | 0x0);            // line #0
  //POWYZEJ DODANE NA POTRZEBY KRZACZENIA PO I2C  
  ...
  else
  {
    ...
    for (uint16_t i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i++) {
      ...
      for (uint8_t x=0; x<16; x++) {
        //Wire.write(buffer[i]);
        I2c.write((uint8_t)_i2caddr, (uint8_t)0x40, (uint8_t)buffer[i]);
        i++;
      }
      i--;
      //Wire.endTransmission();
