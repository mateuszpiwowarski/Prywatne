...
// Auto-generated prototypes for PlatformIO/C++ build
struct ByteBuf;

static inline void rxClearLocked();
static inline void updateObdFuncHeader();
static inline void tftLock();
...
static void showMsg(const char* line1, const char* line2 = nullptr, uint16_t c1 = 0xFEC0, uint16_t c2 = 0xFFFF);
...
static inline bool dashIsPortrait();
...
static void showMsg(const char* line1, const char* line2, uint16_t c1, uint16_t c2) {
...
static void drawOpelSplashZoom(uint32_t ms, int zoomPct, int panX, int panY) {
...
static String fmtFloatPL_noUnit(float v, uint8_t prec) {
...
static void sendCmdLocked(const char* sNoCR) {
  ...
  g_charRX->writeValue((uint8_t*)s.c_str(), s.length(), g_writeWithResponse);
...
static bool query22_bytes(uint16_t did, ByteBuf& outBytes, uint32_t timeout_ms) {
  outBytes.n = 0;
...
static bool query22_u16(uint16_t did, uint16_t& value, uint32_t timeout_ms) {
  value = 0;
  ByteBuf b;
...
static bool query22_u8(uint16_t did, uint8_t& A, uint32_t timeout_ms) {
  A = 0;
  ByteBuf b;
  if (!query22_bytes(did, b, timeout_ms)) return false;
...
static bool query01_u16(uint8_t pid, uint16_t& value, uint32_t timeout_ms) {
  value = 0;
...
static bool query01_u8(uint8_t pid, uint8_t& A, uint32_t timeout_ms) {
  A = 0;
...
static bool bleTargetPresent(const char* mac, int seconds) {
...
static bool probeELMChannel(uint32_t timeoutMs) {
  ...
  for (auto& kv : *svcs) {
    ...
    if (tx && rx) {
      ...
      if (auto* cccd = g_charTX->getDescriptor(BLEUUID((uint16_t)0x2902))) {
        uint8_t v[2] = { static_cast<uint8_t>(g_charTX->canIndicate() ? 0x02 : 0x01), 0x00 };
...
static void dumpConfig() {
  ...
  if (g_charTX) {
    String u = g_charTX->getUUID().toString().c_str();
    Serial.printf("[BLE] TX uuid=%s\n", u.c_str());
  }
  if (g_charRX) {
    String u = g_charRX->getUUID().toString().c_str();
...
static void setActiveProfile(int idx, bool disconnectNow) {
  if (idx < 0 || idx >= PROFILE_COUNT) {
...
static void factoryResetProfile(bool keepMac) {
