...
#define WHEEL_DIAMETER_MM 203           // [mm] 8" hoverboard wheel. Change if your wheel is different.
#define MAX_SPEED_KMH   5               // [km/h] Desired top speed
#define N_MOT_MAX       ((MAX_SPEED_KMH * 5305) / WHEEL_DIAMETER_MM)  // [rpm] Approx. wheel rpm limit derived from wheel diameter
...
*/
  ...
  #else
    #define FLASH_WRITE_KEY     0x1201    // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
    #define DEBUG_SERIAL_USART3           // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
    #undef PRI_INPUT1
    #undef PRI_INPUT2
    #define PRI_INPUT1          0,    0, 0, 4095, 0   // Disabled input. Tie PA2/L_TX to GND so the ADC channel is stable.
    #define PRI_INPUT2          1,    0, 0, 4095, 0   // Active input. Standard potentiometer on PA3/L_RX from 0 to 3.3V.
  #endif
